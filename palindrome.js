...
const isPalindrome = function (str) {
  // normalize to remove spaces and make everything lower case
  const normalized = str.split(' ').join('').toLowerCase()
  const midIndex = Math.floor(normalized.length / 2)
  const lastIndex = normalized.length - 1

  for (let i = 0; i < midIndex; i++) {
    if (normalized[i] !== normalized[lastIndex - i]) return false
  }
  return true
}
